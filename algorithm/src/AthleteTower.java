...
// Check if the prospect can be added to the tower without violating any of the contraint
// This to generate early reject
    private static boolean canAddToPyramid(Human tower, Human prospect) {
        Human cursor = tower;
    ...
    // Core of the algorithm. Recursive method searching linearly for the best tower
// We could search more efficiently if could assume listOhHumans is sorted on their mass first and strength second.
    private static int findBestPyramid_r(Human tower, LinkedList<Human> candidates) {
        int maxSize = tower.size();
//We have exhausted the candidates, nothing to recurse on
        ...
        do {
            ...
            if (canAddToPyramid(tower, prospect)) {
                prospect.previous = tower;
